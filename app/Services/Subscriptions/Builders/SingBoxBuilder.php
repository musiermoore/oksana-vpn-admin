...
class SingBoxBuilder implements SubscriptionBuilder
{
    ...
    public function build(array $nodes): SubscriptionBuildResult
    {
        ...
        $tags = $outbounds->pluck('tag')->values()->all();
        ...
        $payload = [
            'outbounds' => [
                ...
                [
                    'type' => 'selector',
                    'tag' => 'Manual',
                    'outbounds' => array_values(array_unique(['Auto', ...$tags])),
                    'default' => 'Auto',
    ...
    private function buildHysteria2Outbound(array $parsed, string $tag): array
    {
        ...
        if ($parsed['sni'] !== '' || $parsed['insecure']) {
            $outbound['tls'] = array_filter([
                'enabled' => true,
