...
- `/connect-json` использует тот же набор узлов, но отдаёт JSON-профиль с DNS/routing-настройками из конфигурации приложения
- текущие DNS/routing-настройки для `/connect-json` захардкожены в `config/connect_json.php` и вынесены в отдельный provider, чтобы позже их можно было заменить значениями из админки без смены маршрута
